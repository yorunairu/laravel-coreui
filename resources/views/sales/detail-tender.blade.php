...
<li class="breadcrumb-item active">{{ $sales->name }}</li>
@endsection
...
@section('content')
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="progress-indicator-container">
                    <ul class="progress-indicator custom-complex list-unstyled">
                        @foreach ($status_journey as $key => $item)
                            <li class="{{ $sales->status_journey > $item->order ? 'completed' : '' }}">
                                <div class="d-flex align-items-center prgress">
                                    <span
                                        class="bubble {{ $sales->status_journey == $item->order ? 'progress-bar progress-bar-striped progress-bar-animated bg-primary' : '' }}"></span>
                                    <h6 class="ms-2 mb-0">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="col-md-12 text-start">
                                        <h5>Tender Detail</h5>
                                        <hr size="4" noshade color="black" style="padding: 0.5px; background-color: #4D7050;" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 row align-items-center">
                                    <label class="col-sm-3 col-form-label">Tender Type <span class="text-danger">*</span></label>
                                    <div class="col-sm-9">
                                        <div class="form-check form-check-inline">
                                            <input type="radio" id="tender_type_non_direct" name="tender_type" class="form-check-input" value="non_direct" required disabled
                                                {{ $sales->tender_type == 'non_direct' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="tender_type_non_direct">Non Direct</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input type="radio" id="tender_type_direct" name="tender_type" class="form-check-input" value="direct" required disabled
                                                {{ $sales->tender_type == 'direct' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="tender_type_direct">Direct</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input type="radio" id="tender_type_non_tender" name="tender_type" class="form-check-input" value="non_tender" required disabled
                                                {{ $sales->tender_type == 'non_tender' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="tender_type_non_tender">Non Tender</label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-3">
                                        <select class="form-control" data-url="{{ URL::to('select2/get-currency') }}" data-type="currency" name="currency_id" id="currency_id"
                                            data-placeholder="Select Currency" required disabled>
                                            @foreach ($currencies as $key => $item)
                                                <option value="{{ $item->id }}" {{ $item->id == $sales->currency_id ? 'selected' : '' }}>
                                                    {{ $item->currency }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-sm-6">
                                        <input type="text" value="{{ thousandSeparator($sales->total_price_tender) }}" class="form-control" id="total_price_tender" name="total_price_tender"
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="date" class="form-control" id="tanggal_keluar" name="tanggal_keluar" placeholder="Enter Tanggal Keluar"
                                            value="{{ date('Y-m-d', strtotime($sales->tanggal_keluar)) }}" required disabled>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control select2-ajax w-100" data-url="{{ URL::to('select2/cusprin/customer') }}" data-type="customer"
                                            name="customer_id" id="customer_id" data-placeholder="Select Customer" required disabled>
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="email" class="form-control" id="email" name="email" value="{{ $sales->email }}" placeholder="Enter Email" disabled>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="datetime-local" class="form-control" id="deadline" name="deadline" placeholder="Enter Deadline"
                                            value="{{ $sales->deadline }}" required disabled>
                                    </div>
                                </div>
                                ...
                                @if ($sales->status_journey >= 4)
                                    <div class="mb-3 row align-items-center">
                                        <label class="col-sm-3 col-form-label">Win / lost <span class="text-danger">*</span></label>
                                        <div class="col-sm-9">
                                            <div class="form-check form-check-inline">
                                                <input type="radio" id="win_lost_null" name="win_lost" class="form-check-input" value="" required
                                                    {{ $sales->win_lost == '' ? 'checked' : '' }} disabled>
                                                <label class="form-check-label" for="win_lost_null">TBD</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input type="radio" id="win_lost_win" name="win_lost" class="form-check-input" value="win" required
                                                    {{ $sales->win_lost == 'win' ? 'checked' : '' }} disabled>
                                                <label class="form-check-label" for="win_lost_win">Win</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input type="radio" id="win_lost_lost" name="win_lost" class="form-check-input" value="lost" required
                                                    {{ $sales->win_lost == 'lost' ? 'checked' : '' }} disabled>
                                                <label class="form-check-label" for="win_lost_lost">Lost</label>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                            ...
                            <div class="col-md-6">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select" name="is_bb_bond" id="is_bb_bond" disabled>
                                ...
                                <div id="bid_bond_fields" {{ !empty($sales->is_bb_bond) ? '' : 'style="display: none;"' }}>
                                    ...
                                    <div class="row mb-3">
                                        <label for="bb_time_periode" class="col-sm-3 col-form-label">BB Periode to <span class="text-danger">*</span></label>
                                        <div class="col-sm-9">
                                            ...
                                            @if (@$sales->bgGuarantee->where('type', 'bb')->first()->doc_bg)
                                                <p>
                                                    <a class="document-link" href="{{ asset('storage/' . @$sales->bgGuarantee->where('type', 'bb')->first()->doc_bg) }}" target="_blank">
                                                        <i class="fa fa-file document-icon"></i> {{ @$sales->bgGuarantee->where('type', 'bb')->first()->doc_bg }}
                                                    </a>
                                                </p>
                                            @endif
                                        </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select" name="is_pb_bond" id="is_pb_bond" disabled>
                                            <option value="">To Be Decided</option>
                                ...
                                <div id="pb_bond_fields" {{ !empty($sales->is_pb_bond) ? '' : 'style="display: none;"' }}>
                                    ...
                                    <div class="row mb-3">
                                        <label for="pb_time_periode" class="col-sm-3 col-form-label">Time Periode <span class="text-danger">*</span></label>
                                        <div class="col-sm-9">
                                            ...
                                            @if (@$sales->bgGuarantee->where('type', 'pb')->first()->doc_bg)
                                                <p>
                                                    <a class="document-link" href="{{ asset('storage/' . @$sales->bgGuarantee->where('type', 'pb')->first()->doc_bg) }}" target="_blank">
                                                        <i class="fa fa-file document-icon"></i> {{ @$sales->bgGuarantee->where('type', 'pb')->first()->doc_bg }}
                                                    </a>
                                                </p>
                                            @endif
                                        </div>
                    ...
                    <div class="card-body">
                        <div class="text-start">
                            ...
                            @if (in_array($sales->statusJourney->order, [0, 1, 4]))
                                <a href="{{ route('sales.edit', ['id' => encrypt($sales->id), 'type' => 'tender']) }}?is_detail_tender=true"
                                    title="Edit Tender" class="btn btn-warning m-1" target="_blank"><i
                                        class="fa fa-pencil"></i> Edit Tender</a><br>
                            @endif
                            @if (in_array($sales->statusJourney->order, [1, 3, 4]))
                                <a href="{{ route('sales.edit', ['id' => encrypt($sales->id), 'type' => 'material']) }}"
                                    title="Edit Material" class="btn btn-warning m-1" target="_blank"><i
                            ...
                            @if (in_array($sales->statusJourney->order, [0, 1]) && !empty($sales->tenderMaterial->count()))
                                <button type="button" title="Submit to Procurement"
                                    class="btn text-white m-1 btn-primary"
                                    onclick="confirmFinal('{{ route('sales.final', encrypt($sales->id)) }}')"><i
                                        class="fa fa-send-o"></i> Submit to Procurement</button><br>
                            @endif
                            @if ($check_price && in_array($sales->status_journey, ['3']))
                                <a href="{{ route('sales.edit', ['id' => encrypt($sales->id), 'type' => 'material']) }}"
                                    title="Review" class="btn btn-success m-1" target="_blank"><i
                                        class="fa fa-star"></i> Review</a><br>
                            @endif
                            @if (in_array($sales->status_journey, ['4']))
                                <button title="Quotation to Customer" type="button"
                                    class="btn btn-danger m-1 generate-quotation-customer text-light {{ $is_null_bid ? 'bid-warning' : '' }}"
                                    data-id="{{ encrypt($sales->id) }}" data-name="{{ $sales->name }}"
                                    id="generate-quotation-customer"><i class="fa fa-file-pdf"></i> Quotation to
                                    Customer</button><br>
                            @endif
                            {{-- PROCESS TO PP --}}
                            @if (in_array($sales->status_journey, ['4']))
                                <button title="procced to po" type="button"
                                    class="btn btn-primary m-1 procced-to-po {{ $is_null_bid ? 'bid-warning' : '' }}"
                                    data-id="{{ encrypt($sales->id) }}" data-name="{{ $sales->name }}"
                                    id="procced-to-po">
                                    <i class="fa fa-spinner"></i> Procced to PO
                                </button><br>
                                {{-- @else
                            {{ dd($sales->tenderRfq ? $sales->tenderRfq->RfqPrinciple->pluck('status') : 'TenderRfq is null') }} <!-- Debugging nilai status --> --}}
                            @endif
                            {{-- END PROCESS TO PO --}}

                            {{-- PROCESS PO TO PRINCIPLE --}}
                            @if (in_array($sales->statusJourney->order, ['5', '6']))
                                {{-- <button title="po to principle" type="button" class="btn btn-primary m-1 po-to-principle" data-id="{{ encrypt($sales->id) }}" data-name="{{ $sales->name }}" id="procced-to-po"><i class="fa fa-spinner"></i> PO To Principle</button><br> --}}
                                {{-- @else
                            {{ dd($sales->statusJourney->order) }} <!-- Tambahkan ini untuk debug --> --}}
                            @endif
                            {{-- END PROCESS PO TO PRINCIPLE --}}

                            @if (in_array($sales->statusJourney->order, ['4']))
                                <a href="{{ route('sales.activity-list', ['id' => encrypt($sales->id), 'type' => 'tender']) }}?is_detail_tender=true"
                                    title="Edit Tender" class="btn btn-warning m-1" target="_blank"><i
                                        class="fa fa-pencil"></i> Sales Activity</a><br>
                            @endif
                            @if (in_array($sales->statusJourney->order, [0, 1]))
                                <button type="button" title="Delete" class="btn m-1 text-white btn-danger"
                                    onclick="confirmDelete('{{ route('sales.destroy', encrypt($sales->id)) }}')"><i
                            ...
                            <a id="tender-log-modal"
                                ...
                                href="#" title="History Tender" class="btn text-white btn-info m-1"
            ...
            <div class="col-12 mt-3">
                <div class="card">
                    <div class="card-body">
                        <h5>PO Customer</h5>
                        <hr size="4" noshade color="black" style="padding: 0.5px; background-color: #a0a0a0;" />
                            ...
                            <div class="table-responsive">
                                <table class="table w-100 table-hover table-bordered"
                                    ...
                                    <thead class="bg-success">
                                        <th class="text-light text-center v-align-middle">No</th>
                                        <th class="text-light text-start v-align-middle">Tender Name</th>
                                        <th class="text-light text-center v-align-middle">No PO</th>
                                        <th class="text-light text-start v-align-middle">Date</th>
                                        <th class="text-light text-start v-align-middle">By</th>
                                        <th class="text-light text-end v-align-middle">Notes</th>
                                        <th class="text-light text-start v-align-middle">Doc</th>
                                    </thead>
                                    <tbody>
                                    </tbody>
                                </table>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="data-tables">
                            @include('layouts.includes.messages')
                            <div class="table-responsive">
                                <table class="table w-100 table-hover table-bordered" id="table-rfq">
                                    <thead class="bg-success">
                                        <th class="text-light text-center v-align-middle">No</th>
                                        <th class="text-light text-start v-align-middle">Principle</th>
                                        <th class="text-light text-center v-align-middle">Status</th>
                                        <th class="text-light text-start v-align-middle">Reviewed by</th>
                                        <th class="text-light text-start v-align-middle">Reviewed at</th>
                                        <th class="text-light text-start v-align-middle">Date Delivery</th>
                                        <th class="text-light text-end v-align-middle">Price</th>
                                        <th class="text-light text-end v-align-middle">Doc Quo</th>
                                        <th class="text-light text-end v-align-middle">Doc PO</th>
                                    </thead>
                                    <tbody>
                                    </tbody>
                                </table>
                    ...
                    <div class="card-body">
                        ...
                        <div class="data-tables">
                            ...
                            <div class="data-tables">
                                @include('layouts.includes.messages')
                                <div class="table-responsive">
                                    <table class="table w-100 table-bordered table-hover" id="table-material">
                                        <thead>
                                            <tr>
                                                <th rowspan="2" class="text-light text-center v-align-middle bg-secondary">No</th>
                                                <th colspan="4" class="text-light text-center v-align-middle bg-primary">Material
                                                </th>
                                                <th colspan="2" class="text-light text-center v-align-middle bg-warning">Price Unit</th>
                                                <th colspan="2" class="text-light text-center v-align-middle bg-success">Price Total</th>
                                                <th rowspan="2" class="text-light text-end v-align-middle bg-danger">Margin</th>
                                            </tr>
                                            <tr>
                                                <th class="text-light text-start v-align-middle bg-primary">Code</th>
                                                <th class="text-light text-start v-align-middle bg-primary">Description</th>
                                                <th class="text-light text-start v-align-middle bg-primary">Qty</th>
                                                <th class="text-light text-start v-align-middle bg-primary">Uom</th>
                                                <th class="text-light text-end v-align-middle bg-warning">Principle</th>
                                                <th class="text-light text-end v-align-middle bg-warning">KZ</th>
                                                <th class="text-light text-end v-align-middle bg-success">Principle</th>
                                                <th class="text-light text-end v-align-middle bg-success">KZ</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
                                        <tfoot>
                                            <tr>
                                                <td colspan="7" class="text-light text-center v-align-middle font-weight-bold bg-secondary">TOTAL</td>
    ...
    <div class="modal fade" id="quotationPDFModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document"> <!-- Add modal-lg class for a larger modal -->
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="quotationPDFModalLabel">PO : {{ $sales->name }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="quoPDF" class="form-horizontal"
                        ...
                        <button type="submit" class="btn btn-primary" id="saveNewRFQ"><i
                                class="fa fa-file-pdf-o"></i> Print to PDF</button>
                        <button title="Mail Quotation to Sales" type="button"
                            class="btn btn-success m-1 {{ $is_null_bid ? 'bid-warning' : '' }}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="tender-po-principle"
                        action="{{ URL::route('sales.po-customer', ['id' => encrypt($sales->id)]) }}" method="POST"
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-primary" id="savePoPrinciple"><i class="fa fa-save"></i>
                        Submit</button>
                </div>
            </div>
        </div>
    </div>
    <!-- END MODAL PROCCED TO PO -->
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="updatePricePrincipleKzLabel">Update Price From <span
                            class="updatePriceType"></span> : <span class="updatePricePrincipleKz-principle"></span></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                    ...
                    success: function(response) {
                        Swal.fire({
                            ...
                            window.location.href =
                                '{{ route('sales.detail-tender', ['id' => encrypt($sales->id)]) }}';
                        });
                    },
                    error: function(xhr) {
                        Swal.fire({
            ...
            $('#generate-quotation-customer').on('click', function() {
                if ($(this).hasClass('bid-warning')) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Warning !',
                        html: 'Bid Bond should be filled in <a target="_blank" href="{{ route('sales.edit', ['id' => encrypt($sales->id), 'type' => 'tender']) }}" class="btn btn-warning text-white"><i class="fa fa-pencil"></i> Edit Tender</a> first.'
                    });
                } else {
    ...
    <script type="module">
        ...
        $(document).ready(function() {
            $('#table-material').DataTable({
                ...
                footerCallback: function(row, data, start, end, display) {
                    ...
                    function formatCurrency(amount, currency, percentage = null) {
                        ...
                        return '<span class="badge text-end font-weight-bold">' + formattedAmount + '</span>';
                    }

                    function formatCurrencyWithIDR(amount, currency, amount2) {
                        return currency !== 'IDR' ?
                            '<span class="badge text-end font-weight-bold">' + thousandSeparator(amount2) + ' ' +
                            currency + '<br>' + thousandSeparator(amount) + ' IDR</span>' :
                            '<span class="badge text-end font-weight-bold">' + thousandSeparator(amount) +
                            ' IDR</span>';
                    }
                    ...
                    if (totalPrincipleOther > 0) {
                        $(api.column(7).footer()).html(formatCurrencyWithIDR(totalPrinciple,
                            principleCurrency, totalPrincipleOther));
                    } else {
            ...
            $('#table-customer-po').DataTable({
                ...
                "autoWidth": false,
                ajax: {
                    url: "{{ route('sales.po-customer-list', ['id' => encrypt($sales->id)]) }}"
                },
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            Swal.fire({
                                ...
                                window.location.href =
                                    "{{ route('sales.index') }}"; // Redirect to specified URL after SweetAlert
                            });
                        }
                    })
                }
